# Copilot Instructions — Biblia Kids

Biblia Kids es un backend para contenido bíblico educativo e interactivo dirigido a niños y jóvenes. Es **exclusivamente API REST** — sin frontend, sin páginas React, sin CSS ni componentes UI.

## Dominio de negocio

- El contenido central es la Biblia: libros, capítulos y versículos.
- Las experiencias educativas se organizan en historias bíblicas, niveles de dificultad, juegos y planes de lectura.
- El aprendizaje se registra mediante progreso y favoritos por usuario.
- Mantén el dominio limitado al contenido bíblico educativo y al aprendizaje individual de cada usuario.
...
- Autorización basada en permisos cuando el recurso lo requiera; los roles internos actuales son `admin`, `editor` y `user`.
