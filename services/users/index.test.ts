...
describe('user service role assignment', () => {
	...
	it('accepts valid roles independently of tenant context', async () => {
		getAssignableRoles.mockResolvedValue([{ id: ROLE_ID, isSystem: false }]);
		createWithRoles.mockResolvedValue(CREATED_USER);

		await expect(create(BODY, ACTING_USER)).resolves.toMatchObject({ email: BODY.email });
		expect(createWithRoles).toHaveBeenCalledWith(BODY);
	});
});
