...
vi.hoisted(() => {
	...
	process.env.DATABASE_URL = 'postgresql://user:password@localhost:5432/biblia_1?schema=public';
	...
	process.env.OAUTH_MOBILE_CLIENT_ID = 'biblia-kids-mobile';
...
describe('auth mobile login route (ROPC, cliente móvil)', () => {
	...
	it('logs in with username/password using the public mobile client and returns a sessionToken', async () => {
		...
		await handler(buildRequest({ username: 'nino-lector', password: 'biblia-kids123' }), response);

		expect(passwordGrant).toHaveBeenCalledWith('nino-lector', 'biblia-kids123');
		expect(createSession).toHaveBeenCalledWith(
			'11111111-1111-1111-1111-111111111111',
			expect.objectContaining({ accessToken: 'access-token', clientId: 'biblia-kids-mobile' }),
		);
